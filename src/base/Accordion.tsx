 ...
 * Accordion hook
 */
export function useAccordion(config: AccordionConfig) {
  //config
  const { 
    defaultValue,
    onChange,
...
export function AccordionLabel(props: AccordionLabelProps) {
  ...
  //variables
  // extract context
  const { value, itemValue, label, change } = context;
  ...
  return (
    <div
      ...
      style={styles}
      onClick={() => itemValue && change(itemValue)}
...
export function Accordion(props: AccordionProps) {
  ...
  //hooks
  const { activeValue, handlers } = useAccordion({
    defaultValue,
    onChange,
    value
  });
