...
describe('<Scope />', () => {
  ...
  describe('with combined props', () => {
    it(
      'renders children for list, hash, and value together',
      () => {
        const list = [ 'item1' ];
        ...
        expect(screen.getByTestId('scope-key1')).toHaveTextContent(
          'value1'
        );
...
describe('useScopeContext()', () => {
  it(
    'returns default context values when used outside provider',
    () => {
  ...
  it(
    'returns correct context values when used inside provider',
    () => {
      ...
      render(
        <ScopeContext.Provider
          value={{ key: 'testKey', value: 'testValue' }}
        >
          <ContextDisplay />
        </ScopeContext.Provider>
      );
