...
export function MetadataFields(props: FieldsProps<MetadataType>) {
  ...
  //render
  return (
    <div className="frui-form-metadata-row">
      <Input 
        className="frui-form-metadata-name"
      ...
      {input.isText && (
        <Input 
          type={type}
          className="frui-form-metadata-value"
      ...
      {input.isNumber && (
        <NumberInput
          ...
          placeholder={Array.isArray(placeholder) ? placeholder[1]: undefined}
          defaultValue={Array.isArray(value) ? value[1] as string|number: undefined}
          onUpdate={value => handlers.update('value', String(value))}
          error={error}
          required 
        />
      )}
      {input.isDate && type === 'date' && (
        <DateInput 
          ...
          onUpdate={value => {
            handlers.update('value', value ? DateInput.toDateString(value) : '')
          }}
          error={error}
          required 
        />
      ...
      {input.isDate && type === 'time' && (
        <TimeInput 
          ...
          defaultValue={Array.isArray(value) ? value[1]: undefined}
          onUpdate={value => {
            handlers.update('value', value ? TimeInput.toTimeString(value) : '')
          }}
          error={error}
          required 
        />
      ...
      {input.isDate && (type === 'datetime' || type === 'datetime-local') && (
        <DatetimeInput 
          ...
          defaultValue={Array.isArray(value) ? value[1]: undefined}
          onUpdate={value => {
            handlers.update('value', value ? DatetimeInput.toDatetimeString(value) : '')
          }}
          error={error}
          required 
        />
      ...
      <Button 
        ...
        className="frui-form-metadata-remove"
      >
        &times;
      </Button>
