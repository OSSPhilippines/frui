...
describe('<FileInput />', () => {
  ...
  it('calls onUpdate when upload completes', async () => {
    const onUpload = vi.fn(
      (_: File, update: (url: string) => void) =>
        update('uploaded.png')
    );
    const onUpdate = vi.fn();
    render(
      <FileInput onUpload={onUpload} onUpdate={onUpdate} />
    );
    const input = screen.getByTestId(
      'mock-input'
    ) as HTMLInputElement;
  ...
  it(
    'displays custom uploading text when provided',
    async () => {
      const onUpload = vi.fn(
        (_: File, update: (url: string) => void) => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText('Uploading File...')
        ).toBeInTheDocument()
      );
    }
  );
