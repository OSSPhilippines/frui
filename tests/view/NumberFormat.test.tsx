...
import { NumberFormat, formatNumber } from '../../components/view/NumberFormat'
...
describe('<NumberFormat />', () => {
  ...
  it('renders formatted number with a custom separator', () => {
    render(<NumberFormat value={1000000} separator="," decimals={0} />)
    expect(screen.getByText('1,000,000')).toBeInTheDocument()
  })

  it('renders formatted number with a custom decimal symbol', () => {
    render(<NumberFormat value={12.5} decimal="," decimals={1} />)
    expect(screen.getByText('12,5')).toBeInTheDocument()
  })

  it('renders absolute value correctly', () => {
    render(<NumberFormat value={-100.5} absolute decimals={1} />)
